...
export default {
  ...
  methods: {
    ...
    clickSearch(val) {
      this.params.voucherId = val.inputValue;
      console.log("🚀 ~ clickSearch ~ val:", val.inputValue)
    ...
    async exportExcel() {
      let headers = [
        ...
        "商户",
        "核销门店",
        [3, 4].includes(this.type) ? this.amountDes(this.type) : "商户金额",
        "渠道",
        [2].includes(this.type) ? this.amountDes(this.type) : "渠道金额",
        "创建时间",
        "过期时间",
        "操作时间",
        "状态",
      ...
      if ([3, 4].includes(this.type)) {
        ws["!cols"] = ws["!cols"].filter((col, index) => {
          return ![6, 7].includes(index); // 移除渠道和渠道金额列
        });
      }
